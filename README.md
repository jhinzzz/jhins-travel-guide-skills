...
- [`skills/jhins-trip-planner/references/deep/`](./skills/jhins-trip-planner/references/deep/): opt-in deep references (extended tables + examples for `budget` / `dining-rules` / `safety-and-emergency`). LLM reads only when a main reference's pointer crosses a depth trigger.
