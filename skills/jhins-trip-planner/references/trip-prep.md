...
- **Slicing axis**: the itinerary-wide fan-out slices by geography, which here means **country, not topic** — **one sub-agent per country** (the common 2-country trip uses exactly 2). Each sub-agent receives the intake-captured chronic-medication generics (if any).
