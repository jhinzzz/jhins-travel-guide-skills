...
A trip crossing **2+ countries** (or including a long transit country — Schengen transfers, UAE stopovers, Hong Kong transit, etc.) already trips the single fan-out in SKILL.md §Batch Verification. Do **not** serialize visa / entry / payment / SIM / insurance / etiquette research in the main conversation, and do not run a separate trip-prep sweep — these items ride in the geographic slices.

- **Slicing axis**: the itinerary-wide fan-out slices by geography, which here means **country, not topic**. Prep items are country-level, so they attach to **one slice per country** — when the fan-out slices finer than that (2 cities in one country), they ride in the first slice for that country, not in both. Each sub-agent carrying prep items receives the intake-captured chronic-medication generics (if any).
